...
class ScanningViewModel : ViewModel() {
    ...
    private val _mifareUltralightInfo by lazy { MutableLiveData<MifarUltralightInfo>() }
    val mifareUltralightInfo = _mifareUltralightInfo as LiveData<MifarUltralightInfo>
    ...
    fun onActivityNewIntent(intent: Intent?) {
        if (intent?.action == NfcAdapter.ACTION_TECH_DISCOVERED) {
            ...
            if (tag?.techList?.contains(MifareUltralight::class.java.name) == true) {
                ...
                _mifareUltralightInfo.value = MifarUltralightInfo(mfu)
            }
